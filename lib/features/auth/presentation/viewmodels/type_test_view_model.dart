...
final _initialQuestions = <TypeQuestion>[
  /// 익스트림 챌린저
  TypeQuestion(
      questionText: '여행 중에는 땀 흘리며 움직이는 게 가장 즐겁다.', type: TravelType.ACTIVITY),
  TypeQuestion(
      questionText: '즉흥적으로 새로운 체험을 시도하는 걸 좋아한다.', type: TravelType.ACTIVITY),
  TypeQuestion(
      questionText: '아찔하거나 도전적인 액티비티가 끌린다.', type: TravelType.ACTIVITY),

  /// 무드 스포터
  TypeQuestion(
      questionText: '예쁜 카페나 감성적인 공간을 일부러 찾아간다.', type: TravelType.MOOD),
  TypeQuestion(questionText: '인테리어나 분위기를 중요하게 여긴다.', type: TravelType.MOOD),
  TypeQuestion(
      questionText: 'SNS에 공유하고 싶은 공간 사진을 자주 찍는다.', type: TravelType.MOOD),
  ...
  TypeQuestion(
      questionText: '현지 로컬 음식은 꼭 먹어봐야 한다고 생각한다.', type: TravelType.FOOD),

  /// 포레스트 힐러
  TypeQuestion(
      questionText: '복잡한 도시보다 자연에서 쉬는 것이 더 좋다.', type: TravelType.FOREST),
  TypeQuestion(
      questionText: '조용한 풍경 속에서 사진 찍는 것을 즐긴다.', type: TravelType.FOREST),
  TypeQuestion(
      questionText: '여행은 ‘쉼’이 가장 중요하다고 생각한다.', type: TravelType.FOREST),

  /// 컬처 시커
  TypeQuestion(
      questionText: '여행지를 가면 박물관이나 전시를 꼭 간다.', type: TravelType.CULTURE),
  TypeQuestion(
      questionText: '그 지역의 역사나 전통을 알아보는 게 재미있다.', type: TravelType.CULTURE),
  TypeQuestion(
      questionText: '관광보단 로컬 문화를 느끼는 게 더 중요하다.', type: TravelType.CULTURE),
...
class TypeTestViewModel extends _$TypeTestViewModel {
  ...
  Future<void> skipTypeTest() async {
    SkipTypeTestUseCase skipTypeTestUseCase =
        ref.read(skipTypeTestUseCaseProvider);
  ...
  Future<void> submitTypeTestResult(String type) async {
    ...
    try {
      SubmitTypeTestResultUseCase submitTypeTestResultUseCase =
          ref.read(submitTypeTestResultUseCaseProvider);
